...
async def _run_ffmpeg(args, error_class):
    import subprocess
    import sys

    try:
        # Windows-compatible: run ffmpeg in thread pool instead of subprocess_exec
        def _run():
        ...
        if result.returncode != 0:
            raise error_class(result.stderr.decode())
...
async def animate_scene_clip(image_path: Path, scene: SceneDef, output_dir: Path) -> Path:
    ...
    filter_string = build_kenburns_filter(
        effect,
...
async def animate_all_scenes(
    ...
    for sid in scene_ids:
        ...
        if scene:
            tasks.append(
                animate_scene_clip(image_path, scene, output_dir)
            )
...
async def merge_audio_video(
    clip_path: Path,
    scene_manifest: SceneAudioManifest,
    output_dir: Path
) -> Path:
    """Merge animated clip with scene audio (Dialogue + BGM)."""
    ...
    logger.info(
        f"[Animator] Audio synced for scene {scene_manifest.scene_id} "
        f"(BGM: {bool(scene_manifest.bgm_file)})"
    )

    return output_path

async def sync_all_scenes(
    animated_clips: Dict[str, Path],
    scene_image_map: Dict[str, Path],
    ...
    for scene in sorted_scenes:
        ...
        if not scene_manifest:
            logger.warning(
                f"[Animator] No timing manifest for scene "
                f"{scene.scene_id}, skipping"
            )
            continue
        ...
        synced_path = await merge_audio_video(
            clip_path,
            scene_manifest,
            output_dir
        )
        ...
        generated = GeneratedScene(
            ...
            duration_seconds=actual_duration,
            audio_path=Path(scene_manifest.audio_file),
            bgm_path=Path(scene_manifest.bgm_file)
            if scene_manifest.bgm_file else None
        )
