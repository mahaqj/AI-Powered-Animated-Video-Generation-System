...
class MemoryManager:
    ...
    def commit_audio_manifest(self, scene_id: str, audio_tasks: list, metadata: dict = None):
        ...
        Args:
            ...
            audio_tasks: List of audio synthesis tasks for the scene
            metadata: Optional metadata dict
        """
        if metadata is None:
            metadata = {}
        
        # Serialize audio tasks
    ...
    def commit_timing_manifest(self, scene_id: str, timing_entry: dict, metadata: dict = None):
        ...
        Args:
            scene_id: Scene identifier
            timing_entry: Timing manifest entry with audio_file, duration_ms, segments
            metadata: Optional metadata dict
        """
        if metadata is None:
            metadata = {}
        
        # Serialize timing entry
    ...
    def query_audio_by_duration(self, min_duration_ms: int, max_duration_ms: int, n_results: int = 5):
        """Query audio segments by duration range (for finding similar scenes)."""
